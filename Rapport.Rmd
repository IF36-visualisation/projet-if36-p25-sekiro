...
### 8.Existe-t-il un lien entre la fécondité et la mortalité infantile à travers les pays en 2023 ?
à faire
```{r echo=FALSE, message=FALSE, warning=FALSE}
# 1. Charger et nettoyer les données nécessaires
load_and_clean_fertility_mortality <- function() {
  file_path <- "data/Dem_1950-2023.xlsx"
  raw_data <- read_excel(file_path, sheet = "Estimates")
  colnames(raw_data) <- as.character(raw_data[12, ])
  data <- raw_data[-(1:12), ]
...
# 2. Filtrer pour l'année 2023 et pays uniquement
filter_fertility_mortality_2023 <- function(data) {
  data %>%
    filter(grepl("^[A-Za-z]", Country)) %>%
    filter(Year == "2023", !is.na(TFR), !is.na(IMR))
}

# 3. Harmoniser + Ajouter le continent
harmonize_and_add_continent <- function(df) {
  df %>%
    mutate(
      Country = case_when(
        Country == "United States of America" ~ "USA",
...
# 4. Créer le scatter plot interactif
plot_fertility_vs_mortality <- function(df) {
  ggplot(df, aes(x = TFR, y = IMR, color = Continent,
                 text = paste0(Country, 
    ...
    labs(
      title = "Corrélation entre Fécondité et Mortalité Infantile (2023)",
      subtitle = "Chaque point représente un pays. Ligne noire = tendance globale.",
      x = "Taux de Fécondité Total (enfants par femme)",
      y = "Taux de Mortalité Infantile (pour 1 000 naissances vivantes)",
      color = "Continent",
      caption = "Source : World Population Prospects 2024 – ONU"
    ) +
    theme_minimal() +
    theme(
...
fert_mort_data <- load_and_clean_fertility_mortality()
fert_mort_2023 <- filter_fertility_mortality_2023(fert_mort_data) %>%
  harmonize_and_add_continent()

# 6. Visualisation interactive
plot_corr <- ggplotly(plot_fertility_vs_mortality(fert_mort_2023), tooltip = "text")
plot_corr
...
ggplot(combined_data_mirror, aes(x = ValeurAffichee, y = country, fill = ColorGroup)) +
  ...
  labs(
    ...
    subtitle = "Immigration nette à droite | Émigration nette à gauche",
...
* En ce qui concerne la partie exploration nous nous sommes répartis les questions de la manière suivante :
  ...
  + `Zakaria :` **4.** Quels sont les 10 pays représentant la plus forte part de la population mondiale en 2023 ? ,**9.**  Quels sont les 10 pays avec les soldes migratoires nets les plus élevés et les plus faibles                       entre 2000 et 2023 ?  **10.**  Existe-t-il une corrélation entre le taux de mortalité infantile et le taux de fécondité total (TFR) en 2023 ?
