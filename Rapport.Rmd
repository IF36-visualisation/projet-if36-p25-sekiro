...
top10_croissance_mod <- top10_croissance %>%
  mutate(Croissance_M = Croissance / 1000)

# Graphique
ggplot(top10_croissance_mod, aes(x = reorder(Country, Croissance_M), y = Croissance_M)) +
  geom_col(fill = "#2a9d8f") +
  geom_text(aes(label = comma(round(Croissance_M, 1))), hjust = -0.1, size = 4) +
  coord_flip() +
  labs(
    title = "Top 10 des pays à plus forte croissance démographique",
    subtitle = "Période : 2023 à 2100 (scénario moyen - ONU WPP 2024)",
    x = "",
    y = "Croissance de la population (en millions)",
...
## 6. Quel est l'âge médian des populations par pays en 2023 ?
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
# 2. Filtrer que les pays et l'année 2023
...
# 3. Charger les données géographiques du monde (GeoJSON ou shape file simplifié)
load_world_map <- function() {
  map_data("world") %>%
    rename(Country = region)
}

# 4. Fusionner les données géographiques avec les âges médians
prepare_map_data <- function(map_data, median_age_data) {
  left_join(map_data, median_age_data, by = "Country")
}

# 5. Tracer la carte avec ggplot2
plot_median_age_map <- function(map_data) {
  ggplot(map_data, aes(long, lat, group = group, fill = MedianAge, text = paste(Country, "<br>Âge médian:", MedianAge))) +
...
ggplotly(plot_median_age_map(map_final), tooltip = "text")
...
Nous posons l’hypothèse qu’à l’échelle mondiale, l’espérance de vie à la naissance suit une progression quasi-continue depuis 1950. Cette évolution serait principalement due aux avancées médicales, à la généralisation des campagnes de vaccination, à l’amélioration des conditions sanitaires et à l’élévation du niveau de vie dans de nombreuses régions. D’après les projections démographiques, cette tendance devrait se poursuivre jusqu’en 2100, bien qu’à un rythme plus lent. En effet, les gains d’espérance de vie deviennent de plus en plus difficiles à obtenir à mesure que les seuils élevés sont atteints.
